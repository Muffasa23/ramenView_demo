...
const SearchFilter = (props) => {
  ...
  const handleKeyPress = (e) => {
    if(e.key === 'Enter') {
      props.inputOnSubmit(props.mrtFilter, props.tagFilter, props.keyword);
    }
  };
    ...
    <section className="bg-gray-800 px-4 py-3 xl:w-64 xl:py-5">
      ...
      <div className="flex justify-between px-4 py-3 xl:hidden">
        ...
        <button type="button" className="ml-4 inline-flex items-center hover:bg-gray-600 focus:outline-none focus:shadow-outline rounded-lg shadow pl-3 pr-4">
          ...
          <span className="ml-1 text-white font-medium" onClick={ handleFilterOpen } >Filters</span>
        </button>
      </div>

      {/* filter panel */}
      <form className={`xl:block xl:h-full xl:flex xl:flex-col xl:justify-between ${isOpen ? "block" : "hidden"}`}>
